            ...
            <table id="datatablesSimple" class="table table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @foreach ($item->productos as $producto)
                            <p class="text-muted mb-0"> {{ $producto->stock }}</p>
                            @endforeach
                        </td>
                        ...
                        <td>
                            {{ $item->productos->sum('pivot.precio_compra') }}
                        </td>
                        <td>
                            {{ $item->productos->sum('pivot.precio_venta') }}
                        </td>
                        <td>
                            {{ $item->total }}
                        </td>
                        <td>
                            <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                                ...
                                <form action="{{ route('compras.show', ['compra' => $item]) }}" method="get">
                                    <button type="submit" class="btn btn-success" title="Ver">
                                ...
                                <!-- Botón de Editar -->
                                @can('editar-compra')
                                <a href="{{ route('compras.edit', $item->id) }}" class="btn btn-warning" title="Editar">
                                    Editar
                                </a>
                                @endcan
                                ...
                                <!-- Botón de Eliminar -->
                                @can('eliminar-compra')
                                <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#confirmModal-{{$item->id}}" title="Eliminar">
